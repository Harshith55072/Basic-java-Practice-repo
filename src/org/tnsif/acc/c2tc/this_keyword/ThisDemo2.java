package org.tnsif.acc.c2tc.this_keyword;

// "this" used to refer to the current object's own field/method,
// disambiguating it from the constructor parameter of the same name.
class ProductBottle {
    double price;

    ProductBottle(double price) {
        this.price = price;
    }

    double calculateDiscount() {
        return this.price * 0.10;
    }

    double calculateFinalPrice() {
        return this.price - this.calculateDiscount();
    }
}

public class ThisDemo2 {

    public static void main(String[] args) {
        ProductBottle product = new ProductBottle(400);
        System.out.println("Final Price :" + product.calculateFinalPrice());
    }
}
